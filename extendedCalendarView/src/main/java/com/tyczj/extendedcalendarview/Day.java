...
public class Day{
	...
	int year;
	int month;
    EventLoaderListener loaderListener;
	Context context;
	BaseAdapter adapter;
	...
	private class GetEvents extends AsyncTask<Void,Void,Void>{
		...
		protected Void doInBackground(Void... params) {
            if (loaderListener != null) {
                ...
                ArrayList<Event> loaderEvents = loaderListener.loadEvents(cal.getTimeInMillis());
                for (int i = 0; i < loaderEvents.size(); i++) {
                    events.add(loaderEvents.get(i));
                }
            }
            else {
